...
class Historia_clinica extends Odon_Controller {
    ...
    public function seleccionar_cita(){
        $this->session->set_userdata(array('id_cliente' => $_POST['id_cliente']));
        $this->session->set_userdata(array('id_cita' => $_POST['id_cita']));
    }
    
    public function eliminar_seleccion(){
        $this->session->set_userdata(array('id_cliente' => null));
        $this->session->set_userdata(array('id_cita' => null));
        if($this->session->userdata['id_cliente'] == null){
            redirect('Odontologo/Cita');
        }
    }
    ...
    public function crear_historia_clinica(){
      ...
      public function nueva_historia_clinica(){
          ...
          $this->form_validation->set_rules('input_antecedentes', 'antecedentes', 'required', array('required' => 'Escriba los antecedentes familiares.'));
          $this->form_validation->set_rules('input_enfermedad', 'enfermedad', 'required', array('required' => 'Escriba las enfermedades que padece el paciente.'));
          $this->form_validation->set_rules('input_observaciones', 'observaciones', 'required', array('required' => 'Escriba las observaciones que haya realizado.'));
          ...
          if ($this->form_validation->run()) {
              ...
              if($result){
                  ...
                  for($i = 1; $i <= 36; $i++){
                      ...
                      $this->pregunta_model->insertar_preguntas($input);
                  }
              }
